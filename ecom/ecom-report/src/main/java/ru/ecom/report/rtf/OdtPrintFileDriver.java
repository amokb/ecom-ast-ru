...
public class OdtPrintFileDriver implements IPrintFileDriver, IQRPrinter {
    ...
    private static void setTextByTag(Element aEl, String aText) {
        ...
        for (int i = 0; i < sp.length; i++) {
            aEl.addContent(sp[i]);
            if (i < (sp.length - 1))
                aEl.addContent(new Element("line-break", "text", "urn:oasis:names:tc:opendocument:xmlns:text:1.0"));
        }
    }
        ...
        if (aElement.getText().contains(theReplaceString)) {
            ...
            if (index1 != -1 && index2 != -1) {
                ...
                if (pars.length == 3) {
                    ...
                    try {
                        int qr_w = Integer.parseInt(pars[1]);
                        int qr_h = Integer.parseInt(pars[2]);
                        if (qr_w != 0 && qr_h != 0 && !qrText.equals("")) {
                            theQR_text = qrText;
                            theQR_w = qr_w;
                            theQR_h = qr_h;
    ...
    static boolean isEndFor(String aValue) {
        return "$$FOREND".equals(aValue);
    }
     ...
     * Строка для замены
     */
    private static final String theReplaceString = "printQRCode";
}