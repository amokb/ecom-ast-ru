...
public class DepartmentMedCase extends HospitalMedCase {
    ...
    public Diagnosis getMainDiagnosis() {
        for (Diagnosis diagnosis : getDiagnoses()) {
            if ("1".equals(diagnosis.getPriority().getCode()) && "4".equals(diagnosis.getRegistrationType().getCode()))
                return diagnosis;
        }
        return null;
    }
    ...
    @OneToOne
    public MisLpu getTransferDepartment() {
        return theTransferDepartment;
    }

    public void setTransferDepartment(MisLpu aTransferDepartment) {
        theTransferDepartment = aTransferDepartment;
    }
    ...
    @OneToOne
    public MedCase getPrevMedCase() {
        return thePrevMedCase;
    }

    public void setPrevMedCase(MedCase aPrevMedCase) {
        thePrevMedCase = aPrevMedCase;
    }
    ...
    @OneToOne
    public BedFund getBedFund() {
        return theBedFund;
    }

    public void setBedFund(BedFund aBedFund) {
        theBedFund = aBedFund;
    }
    ...
    @Comment("№ палаты")
    @OneToOne
    public HospitalRoom getRoomNumber() {
        return theRoomNumber;
    }

    public void setRoomNumber(HospitalRoom aRoomNumber) {
    ...
    @Comment("№ койки")
    @OneToOne
    public HospitalBed getBedNumber() {
        return theBedNumber;
    }

    public void setBedNumber(HospitalBed aBedNumber) {
        theBedNumber = aBedNumber;
    }
    ...
    public String getDaysCount() {
        return DurationUtil.getDurationMedCase(getDateStart(),
                getTransferDate() != null ? getTransferDate() : getDateFinish(), 1);
    }
