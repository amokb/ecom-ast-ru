...
public class WorkCalendarHospitalBedSave implements IFormInterceptor {
    public void intercept(IEntityForm aForm, Object aEntity, InterceptorContext aContext) {
		...
		if (form.getInternalCode()==null || form.getInternalCode().equals("")){//Генерация номера направления
			...
			String defaultLpuId = getClazz(ISoftConfigService.class).getDefaultParameterByConfig("DEFAULT_LPU",null,manager);
	...
	private <T>T getClazz(Class<T> clazz) {
		return EjbInjection.getInstance().getRemoteService(clazz);
	}
}
