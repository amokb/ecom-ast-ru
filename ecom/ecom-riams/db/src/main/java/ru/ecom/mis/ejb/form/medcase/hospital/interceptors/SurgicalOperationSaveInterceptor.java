		...
		if (needPaidConfirmation || isCalcDogovor) {
			...
			if (needPaidConfirmation) { //ищем подтверждение оплаты
				...
				if (letter == null) {
					...
					EjbInjection.getInstance().getRemoteService(IContractService.class) //todo make norm
					.addMedServiceAccount(typeService, oper.getId(), medServiceCode, patientId, letter, manager);
				}
			}
		}
    }
